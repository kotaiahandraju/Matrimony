...
var listOrders1 = ${pref_loc_profiles};
...
function displayMatches(listOrders) {
	...
	$.each(listOrders,function(i, orderObj) 
	{
		...
		var shortListedStr = '<span id="shortlistloc'+orderObj.id+'"><a href="#no" type="button" class="btn" style="padding:5px; color:blue; border-radius:5px;" onclick="shortList_dashboard_loc('+orderObj.id+')"> Shortlist</a></span>';
		...
		var interestStr = "";
		if(expressed==0){
			interestStr = '<span id="expInterestloc'+orderObj.id+'"><a   href="#no" type="button" class="btn" style="padding:5px; color:blue; border-radius:5px;" onclick="expressInterest_dashboard_Pref('+orderObj.id+')">  Express Interest  </a></span>';
		}else if(expressed>0){
...
function expressInterest_dashboard_Pref(profile_id){
...
// 	    				alert("Interest request has been sent successfully");
								...
								$('#expressModal').show();
			    				$('#expressModal').modal();
	    				$("#expInterestloc"+profile_id).html('Expressed Interest');
	    				$("#expInterestloc"+profile_id).prop("disabled",true);
...
function shortList_dashboard_loc(profileId){
	$("#id").val(profileId);
	...
	jQuery.fn.makeMultipartRequest('POST', 'shortList', false,
    		...
    		if(typeof msg != "undefined"){
    			if(msg=="success"){
    				...
    				$("#member_name_todisplay").html(" to "+profileObj.firstName+" "+profileObj.lastName);
    				$("#shortlistloc"+profileId).html('Shortlisted');
    				//$("#shortlistPref"+profileId).removeAttr("href");
    				//$("#shortlistPref"+profileId).attr("disabled");
    			}else{
