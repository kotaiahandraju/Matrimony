					...
					<form:form modelAttribute="rassiForm" class="form-horizontal" role="form" id="rassi-form" action="addRassi" method="post">								
						<div class="form-group">
							<div class="col-sm-12">
								...
								<div class="errorMessage" id="Rassi_invalid_em_" style="display:none"></div>
							</div>
					  	</div>
						<div class="form-group">
							<label class="col-sm-4 control-label required"><spring:message code="label.rassiName" text="default text" /><span style="color:red;">*</span></label>
							<div class="col-sm-8">
								<form:hidden path="id"/>
						  		<form:input path="name" type="text" class="form-control onlyCharacters validate"  autocomplete="off" placeholder="Enter Raasi" maxlength="255"/>						
...
function validate(id){
	if($('#name').val() ==  null || $('#name').val() == ""  || $('#name').val()=="undefined" ) {
	    ...
	    $("#nameError").text("Raasi cannot be blank.");
	}else{
		$("#nameError").text("");
	}
	}
 ...
 */
 ...
 function displayTable(listOrders) {
		...
		$.each(listOrders,function(i, orderObj) {
						var edit = "<a onclick='editRaasi("+ orderObj.id+ ")'><i style='color: green;' class='fa fa-edit'></i></a>"
						var deleterow = "<a onclick='deleteRaasi("+ orderObj.id+ ")'><i style='color: red;' class='fa fa-trash'></i></a>"
...
//		$(window).scrollTop($('#addForm').offset().top);
 ...
 function deleteRaasi(id){
		...
		$.fn.makeMultipartRequest('POST', 'deleteRaasi', false,
