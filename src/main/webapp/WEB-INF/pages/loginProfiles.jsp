	...
	<div class="breadcrumbs">
		<ul>
			...
			<li>
				...
				<span style="color: #999;cursor: auto;">Today Login Profiles</span>
			</li>
		</ul>
		...
		<div class="box">
			...
			<div class="box-content nopadding table-responsive w3-animate-zoom" id="tableId">
				<table id="tableToExport" class="table table-hover table-nomargin table-bordered dataTable dataTable-column_filter" data-column_filter_types="text,text,text,text,text,null">
					<thead>
					<tr>
						<th>Login time</th><th>UserName</th><th>First Name</th><th>Last Name</th><th>Email</th><th>Mobile</th><th>Profile Status</th>
						<th></th>
					</tr>
					</thead>
					<tbody></tbody>
		...
		$.each(listOrders,function(i, orderObj) {
			var viewProfile = "<a data-toggle='tooltip' title='View' onclick='viewProfile("+ orderObj.id+ ")'><i style='color: #3c8dbc;cursor: pointer;' class='fa fa-eye'></i></a>"
							...
							if(orderObj.firstName !=null){
								if(orderObj.loginActive == '1'){
									var username = "<td style='color: green;' title='"+orderObj.username+"'><b>" + orderObj.username + "</td>"
								}else{
									var username = "<td title='"+orderObj.username+"'>" + orderObj.username + "</td>"
								}
								
							var tblRow = "<tr >"
								+ "<td title='"+orderObj.last_login_time+"'>" + orderObj.last_login_time + "</td>"
								+ username
								...
								+ "<td title='"+orderObj.profileStatus+"'>" + orderObj.profileStatus + "</td>"
								
								+ "<td style='text-align: center;white-space: nowrap;'>" + viewProfile 
								+ "</td>"  
								+ "</tr >";
