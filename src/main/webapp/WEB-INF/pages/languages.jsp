						...
						<div class="form-group">
							<div class="col-sm-12">
								<c:if test="${not empty msg}">
									<div class="alert alert-success fadeOut animated">${msg}</div>
								</c:if>
								<div class="errorMessage" id="Languages_invalid_em_"></div>
							</div>
					  	</div>
						<div class="form-group">
							<label class="col-sm-4 control-label required"><spring:message code="label.languageName" text="default text" /> <span style="color:red;">*</span></label>
							<div class="col-sm-8">
								<form:hidden path="id"/>
...
function validate(id){
	if($('#name').val() ==  null || $('#name').val() == ""  || $('#name').val()=="undefined" ) {
	    ...
	    $("#nameError").text("Language cannot be blank.");
	}else{
		$("#nameError").text("");
	}
	}
 ...
 */
 ...
 function displayTable(listOrders) {
		...
		$.each(listOrders,function(i, orderObj) {
						var edit = "<a onclick='editLanguage("+ orderObj.id+ ")'><i style='color: green;' class='fa fa-edit'></i></a>"
						var deleterow = "<a onclick='deleteLanguage("+ orderObj.id+ ")'><i style='color: red;' class='fa fa-trash'></i></a>"
...
//		$(window).scrollTop($('#addForm').offset().top);
 ...
 function deleteLanguage(id){
		...
		$.fn.makeMultipartRequest('POST', 'deleteLanguage', false,
