...
var listOrders1 = ${latest_updated_profiles};
...
function displayMatches(listOrders) {
	...
	$.each(listOrders,function(i, orderObj) 
	{
		...
		var shortListedStr = '<span id="shortlistUpdate'+orderObj.id+'"><a href="#no" type="button" class="btn" style="padding:5px; color:blue; border-radius:5px;" onclick="shortList_dashboard_update('+orderObj.id+')"> Shortlist</a></span>';
		...
		var interestStr = "";
		if(expressed==0){
			interestStr = '<span id="expInterestUpdate'+orderObj.id+'"><a   href="#no" type="button" class="btn" style="padding:5px; color:blue; border-radius:5px;" onclick="expressInterest_dashboard_update('+orderObj.id+')">  Express Interest  </a></span>';
		}else if(expressed>0){
...
function expressInterest_dashboard_update(profile_id){
...
// 	    				alert("Interest request has been sent successfully");
						$("#profile_id").val(profile_id);
								...
								$('#expressModal').show();
			    				$('#expressModal').modal();
	    				$("#expInterestUpdate"+profile_id).html('Expressed Interest');
	    				$("#expInterestUpdate"+profile_id).prop("disabled",true);
...
function shortList_dashboard_update(profileId){
	$("#id").val(profileId);
	...
	jQuery.fn.makeMultipartRequest('POST', 'shortList', false,
    		...
    		if(typeof msg != "undefined"){
    			if(msg=="success"){
    				...
    				$("#shortlistUpdate"+profileId).html('Shortlisted');
